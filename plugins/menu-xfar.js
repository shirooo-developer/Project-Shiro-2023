...
let handler = async (m, { conn, args, text, usedPrefix, command }) => {
    ...
    if (args[0] == 'scdl') {
        await xa.SoundCloud(args[1]).then(async (data) => {
            ...
            conn.sendMessage(m.chat, { image: { url: data.thumbnail }, jpegThumbnail:buf, caption: `${txt}` }, { quoted: m, sendEphemeral: true })
            conn.sendMessage(m.chat, { audio: { url: data.medias[0].url }, mimetype: 'audio/mpeg', fileName: data.title+'.m4a' }, { quoted: m })
        }).catch((err) => {
            conn.reply(m.chat, `*Failed to download media and send audio*`, m)
        })
    } else if (args[0] == 'twit') {
        await xa.Twitter(args[1]).then(async (data) => {
            ...
            conn.reply(m.chat, `*Failed to download media and send videos*`, m)
        })
    } else if (args[0] == 'fb') {
        await xa.Facebook(args[1]).then(async (data) => {
            ...
            conn.reply(m.chat, `*Failed When Downloading Media and Sending Files*`, m)
        })
    } else {
        conn.reply(m.chat, `*Invalid Options, Options are scdl, twit, fb*`, m)
    }
}

handler.help = ['xfr [option] [link]']
